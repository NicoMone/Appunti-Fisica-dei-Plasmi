...
\epigraph{
	Riferimenti: cap. 7 di \emph{Fisica dei plasmi} di Pucella-Segre
}{}
...
Finora abbiamo trovato le equazioni di bilancio del numero di particelle (equazione di continuità), equazione della quantità di moto (o impulso) ed equazione dell'energia per le varie specie che compongono il plasma, ma nel caso più semplice di due fluidi (ioni + elettroni) possiamo fare alcune definizioni per ricondurci ad una descrizione a un fluido, la \underline{magnetoidrodinamica} (\text{MHD} da \emph{Magneto-Hydro-Dynamics}); descrizione che spazia su molti ordini di grandezza ma perdendo alcuni aspetti cinetici. 

Questa teoria è valida alle seguenti condizioni:
\begin{itemize}
	\item il moto è fortemente collettivo $\iff$ molte particelle in un volume di Debye ($n\, \lambda_D^2 \gg 1$);
...
\[ 
\begin{split}
	\textbf{numero di particelle} &:\qquad \qquad n_i\ \sim\ n_e\ \sim\ \frac{n}{2} \\
	\textbf{densità di massa} &:\qquad \qquad \rho\ =\ \sum_\alpha m_\alpha n_\alpha = m_i n_i + m_e n_e \sim \frac{n}{2}(m_i + m_e) \\
	\textbf{velocità media} &:\qquad \qquad \bar u\ =\ \frac{\sum_\alpha m_\alpha n_\alpha u_\alpha}{\sum_\alpha m_\alpha n_\alpha}\sim \frac{2n}{2n} \frac{m_i \vec u_i + m_e \vec u_e }{m_i + m_e}\ \sim\ \vec u_i + \frac{m_e}{m_i} \vec u_e \\
	\textbf{corrente} &:\qquad \qquad \vec J\ =\ \sum_\alpha q_\alpha n_\alpha \vec u_\alpha
\end{split}
\]
...
\vspace{0.5cm}
\section{``Assembliamo'' la magnetoidrodinamica}

\subsection{Equazione di continuità ad un fluido}

Ricordiamo che l'equazione di continuità per una singola specie è $\frac{\partial n_\alpha}{\partial t} = -\vec \nabla\cdot (n_\alpha \vec u_\alpha)$, nel framework della teoria a due fluidi che abbiamo sviluppato. 

Partiamo da quest'ultima, moltiplicandola per $m_\alpha$ e facciamo una somma sulle specie per ottenere una nuova equazione di continuità a singolo fluido:
...
Abbiamo ottenuto l'\underline{equazione di continuità ad un fluido}.

\vspace{0.5cm}

In modo del tutto simile potremmo definire l'\underline{equazione di continuità di carica}, avendo prima definito la densità di carica $\rho_c=\sum_\alpha n_\alpha q_\alpha$:
...
Usiamo $\langle \vec v_\alpha\rangle = \vec u_\alpha$ e calcoliamo $\mathbf{P}=\sum_\alpha \mathbf{P}_\alpha$:
...
&=\ \rho\,  \frac{\partial \vec u}{\partial t}\ +\ \cancelto{\text{eq. di continuità}}{\vec u  \frac{\partial\rho}{\partial t}\ +\ \vec u\vec\nabla\cdot (\rho\vec u)}\ +\ \rho(\vec u\cdot \vec \nabla)\vec u\ +\ \vec \nabla \cdot \mathbf{P} = \\ 
...
dove $\rho_c$ è uno sbilancio di carica quindi è vicino a zero, ma non nullo. In definitiva, aver sommato le due equazioni del moto fluide ci ha portato alla forma in eq.(\ref{moto MHD}), un'\textbf{equazione del moto a singolo fluido}:
...
%
\vspace{0.5cm}
\paragraph{Faraday in MHD : } 

Consideriamo la legge di Faraday in forma locale $\frac{1}{c}\, \frac{\partial \vec B}{\partial t}\, =\, -\vec \nabla \times \vec E$, dimensionalmente abbiamo che $B\, /\, c\tau \sim E\, / L$ , quindi $\frac{E}{B}\, \sim\, \frac{L}{c\, \tau}\,\sim\, \frac{u}{c}$ , così che abbiamo una stima di una velocità caratteristica ($u\sim L/\tau$), funzione di lunghezza e tempo caratteristici, rispetto ai campi, questa considerazione ci servirà più avanti.

Nella MHD ideale andiamo a supporre che $\vec E\, +\, \frac{\vec u\times \vec B}{c}\, =\, 0$ (come chiedere che nel sistema di riferimento del fluido si annulli il termine e.m. nell'equazione del moto) quindi, in unità $c=1$, vediamo che l'equazione di \underline{Faraday-Maxwell in MHD ideale} assume la forma:
...
%
\vspace{0.5cm}
\paragraph{Ampére in MHD : } 

Dalla legge di Ampère sappiamo che $\frac{1}{c}\frac{\partial E}{\partial t}\, =\, \vec \nabla \times \vec B\, -\, \frac{4\pi}{c} \vec j$, andiamo a fare ancora un'analisi dimensionale per capire quale termine dominini:

\[ \Rightarrow\qquad\quad \frac{|\partial_t\, \vec E|}{c\,|\vec \nabla\, \times\, \vec B|}\ \sim\ \frac{E\, L}{c\, \tau\, B}\ \sim\ \left( \frac{u}{c}\right)^2\ \ , \]

ciò ci permette di dire che nel caso non-relativistico in cui $u\ll c$, il termine $\partial_t \vec E$ è trascurabile rispetto al termine $\vec \nabla \times\vec B$, quindi l'equazione di \underline{Ampère-Maxwell in MHD} diventa: 
\begin{equation}
	\label{Ampére MHD}
...
%
\vspace{0.5cm}
\paragraph{Equazione del moto in MHD : } 
Stiamo pian piano costruendo questo nostro modello della magnetoidrodinamica, riconsideriamo ora l'eq. del moto eq.(\ref{moto MHD}). 
...
%
\vspace{0.5cm}
\subsection{Arriviamo all'equazione di Ohm generalizzata}
...
Si nota che siamo partiti da 2 equazioni quindi possiamo ricavarne altrettante, c'è ancora dell'informazione da estrarre dall'eq.(\ref{moto}) del moto qui sopra riportata. 

Dopo che siamo andati a fare la somma sulle specie per trovare l'equazione del moto del singolo fluido, se moltiplichiamo per $q_\alpha$ e quindi facciamo la differenza delle singole equazioni del moto delle due specie, quella che troveremo sarà l'equazione di Ohm generalizzata. 

\emph{Moltiplichiamo l'eq. del moto fluida di singola specie per $q_\alpha$, per poi sommare sulle specie $\alpha$}:

\begin{equation}\label{precursore Ohm}
\underset{\partial \vec j/\partial t}{\underbrace{\sum_\alpha \frac{\partial(q_\alpha n_\alpha \vec u_\alpha)}{\partial t} }}\ +\ \sum_\alpha\, q_\alpha\, \nabla \cdot (n_\alpha \langle \vec v \vec v\rangle_\alpha)\ -\ \sum_\alpha\, q^2_\alpha \,\frac{n_\alpha}{m_\alpha}\, \left(\vec E + \frac{1}{c}\vec u_\alpha\times \vec B\right)\ =\ 0
\end{equation}

Per il secondo termine torniamo a giocare con la definizione di velocità peculiare:

\[ \begin{aligned} \langle \vec v_\alpha \vec v_\alpha\rangle\ =\ \langle (\vec u + \vec w_\alpha)(\vec u + \vec w_\alpha) \rangle\ &=\ \langle \vec w_\alpha \vec w_\alpha\rangle\, +\, \langle \vec w_\alpha \vec u\rangle\, +\, \langle \vec u \vec w_\alpha\rangle \, +\, \vec u\vec u\ =\\ &=\ \langle \vec w \vec w\rangle_\alpha \,+\, (\vec u_\alpha - \vec u)\, \vec u \, +\, \vec u\, (\vec u_\alpha - \vec u)\, +\, \vec u\vec u\ =\\
	&=\ \langle \vec w \vec w\rangle_\alpha\, +\, 2\vec u_\alpha \vec u\, +\, \vec u \vec u\ \ .
\end{aligned}\]

Si faccia attenzione al fatto che $\vec v_\alpha$ e $\vec w_\alpha$ sono legati dalla velocità media del fluido $\vec u$ ($\vec v_\alpha = \vec u + \vec w_\alpha$), quindi nei calcoli abbiamo usato la relazione $\vec w_\alpha = \vec v_\alpha + \vec u$ assieme alla media di specie $\langle \vec v_\alpha\rangle =\vec u_\alpha$. 


\subsubsection{Plasma di $e^-$ e $p$: sviluppo della Ohm generalizzata}
...
\[\begin{cases}
	n_e\ \sim\ n_i\ \sim\ n / 2\\
	\vec j\ =\ \sum_\alpha q_\alpha n_\alpha \vec u_\alpha\ =\ n\,e\, (\vec u_i-\vec u_e)\, ;\\
	\rho \vec u\ =\ \sum_\alpha n_\alpha m_\alpha \vec u_\alpha\\
	\rho \sim n\, m_i
\end{cases}
\quad \rightarrow \quad
\vec u \sim \vec u_i + \frac{m_e}{m_i} \vec u_e
 \quad \rightarrow \quad
\begin{cases}
	\vec u_i = \vec u +\frac{m_e}{m_i} \frac{\vec j}{n\, e}\\
	\vec u_e = \vec u - \frac{j}{n\, e}
\end{cases}\]

Usiamo queste ipotesi nell'eq. \ref{precursore Ohm}, nella quale abbiamo già riconosciuto la corrente $\vec j$:
...
\[\begin{aligned}
\sum_\alpha n_\alpha q_\alpha \langle vv\rangle_\alpha 
&=\ ne \langle \vec v\vec v\rangle_i - ne \langle \vec v\vec v\rangle_e\ =\\
...
&=\ n e \left( \underset{\text{press. singola specie } \langle w w \rangle_i}{\underbrace{\langle (\vec v_i -\vec u)\, (\vec v_i - \vec u) \rangle_i}} + \cancel{\vec u\vec u} + \langle (\vec v_i - \vec u)\, \vec u \rangle_i + \underset{\vec u\langle v_i - \vec u\rangle_i=\vec u(\vec u_i-\vec u)}{\underbrace{\langle \vec u (\vec v_i - \vec u)\rangle_i}}\right)\, -\, ne\,(\dots)_e\,
\end{aligned}\]

da cui, in definitiva, considerando i termini che si elidono fra $+ne(\dots)$ e $-ne(\dots)$, inserendo le espressioni di $\vec u_i$ e $\vec u_e$ troviamo che il secondo termine a meno del gradiente diviene:
...
Quindi, per ora, sviluppando eq. \ref{precursore Ohm} abbiamo trovato una forma intermedia:

\[ \frac{\partial \vec j}{\partial t} + \nabla \cdot \left[ e\, \left(\frac{p_i}{m_i}- \frac{p_e}{m_e}\right)\, +\,  \vec j \vec u + \vec u\vec j\ \right] \,-\, \sum_\alpha\, q^2_\alpha \,\frac{n_\alpha}{m_\alpha} (\vec E + \frac{1}{c}\vec u_\alpha\times \vec B) \ =\ 0\ \ . \]

Prendiamo infine il terzo termine con $q_\alpha^2$, anche qui inseriamo :

\[\begin{aligned}
	\sum_\alpha \frac{n_\alpha}{m_\alpha}\, q^2_\alpha\, \left(\vec E + \frac{\vec u_\alpha\times \vec B}{c}\right)
	\ &=\ n\, e^2\, \left(\cancel{\frac{1}{m_i}}\, +\, \frac{1}{m_i}\right)\vec E\, +\, n\,e^2\, \left(\sum_\alpha \frac{\vec u_\alpha}{m_\alpha}\right)\times \vec B/c\ =  \\ 
	&=\ \frac{n\, e^2}{m_e} \vec E\, +\, \frac{n\, e^2}{m_e}\, \left(\vec u \, + \,\frac{\vec j}{m_e}\right)\times \frac{\vec B}{c}
\end{aligned}\]

dove abbiamo nascosto sotto il tappeto il passaggio intermedio:

\[ \sum_\alpha\, \frac{\vec u_\alpha}{m_\alpha}
\ =\ \frac{\vec u}{m_i}\, +\, \frac{m_e \, \vec j}{m_i^2\, n_e}\, +\, \frac{\vec u}{m_e}\, -\,\frac{\vec j}{m_e\, n_e}
\ =\ \frac{1}{m_e}\, [\vec u\, (1+ \cancelto{\sim 0}{\frac{m_e}{m_i}})\, -\, \frac{\vec j}{n_e}\, (1+\cancelto{\sim 0}{\frac{m_e}{m_i}} ) ]\ . \]

Otteniamo così per il terzo termine:

\[\sum_\alpha \frac{n_\alpha q^2_\alpha}{m_\alpha} \left(\vec E + \frac{\vec u_\alpha \times \vec B}{c}\right)\
\approx\ \frac{n\, e^2}{m_e} \left(\vec E + \frac{1}{c} \vec u\times \vec B - \frac{1}{n\, e\, c} \vec j\times \vec B\right)
\]
...
%
%
\subsection{Equazione di Ohm ideale vs. resistiva} 

Ripartiamo dall'equazione di Ohm generalizzata, ma riarrangiata rispetto ad eq.(\ref{eq di Ohm generalizzata}) e questa volta non ideale quindi inserendo il termine resistivo:
...
\[ \frac{\ |\vec\nabla p_i|\,/\, n\,e\ }{\ |\vec u\times \vec B|\,/\,c\ }\ \sim\ \frac{m_i\, n\, v_i^2}{L\, n\, e}\frac{c}{u\, B}\quad \underset{v_i\sim u}{\sim}\quad \frac{m_i\, c\, u}{e\, B\, L}\ \sim \ \frac{\omega}{\omega_{cicl.}}\ \ . \]
...
quindi per la frequenza di ciclotrone vale che 
$\omega_{cicl.} = v_A\, \omega_{p,\, i}/c = v_A/\delta_i$, dove $\delta_i\, =\, c\, /\, \omega_{p,\, i}$ è la lunghezza pelle degli ioni che avevamo già definito in passato come $\delta_s = \displaystyle\sqrt{\frac{m_s}{4\pi\, n\, q_s^2}}\, c$.
...
che è la nostra \underline{equazione di Ohm in MHD ideale} (che abbiamo usato già precedentemente vedi per la forma di Faraday MHD), se invece teniamo un termine resistivo abbiamo:

\begin{equation}
	\label{Ohm resistiva}
	\vec E\, +\, \frac{\vec u\times \vec B}{c}\ =\ \eta \vec j \ .
\end{equation}

detta \underline{equazione di Ohm resistiva}.

Un'altra versione accennata a lezione è la \emph{'electron MHD Ohm's law'}: $\vec E\, +\, \frac{\vec u_e\times \vec B}{c}\ =\ 0$ .


%
%
\vspace{0.5cm}
\subsection{Equazione dell'energia di singolo fluido}
...
Guardando al primo termine vediamo che questo rappresenta ora la variazione dell'energia interna $\ \partial_t\, \left( \rho\, u^2\, /\, 2\ +\ 3\, p\, /\, 2 \right)\ $, sappiamo che $P_\alpha = \frac{1}{3} m_\alpha n_\alpha \langle \vec w\vec w \rangle = n_\alpha T_\alpha$ e troviamo:
...
\[\langle \vec w\rangle_\alpha\ =\ \vec u_\alpha\, +\, \vec u \qquad \iff \qquad \vec v_\alpha\ =\ \vec u\, +\, \vec w_\alpha\ \ ,\]
quindi non è più a media nulla. Cerchiamo ora di calcolare $\langle v^2 \vec v\rangle_\alpha$:

\[ \begin{aligned}\langle\, v^2\, \vec v\,\rangle_\alpha\ 
&=\ \langle\ [\ (\vec w +\vec u)\cdot(\vec w + \vec u)\ ]\ (\vec w + \vec u)\ \rangle_\alpha \\ 
...
\[ \frac{1}{2} \sum_\alpha n_\alpha m_\alpha u^2 \vec u\ =\ \frac{1}{2} \rho u^2 \vec u\]

\[ \frac{1}{2} \sum_\alpha n_\alpha m_\alpha\,\langle u^2 \vec w_\alpha \rangle = \frac{1}{2} \sum_\alpha n_\alpha m_\alpha\,u^2 \langle \vec w \rangle_\alpha\ =\ \frac{1}{2} \sum_\alpha n_\alpha m_\alpha\, u^2\ \langle\, \vec u_\alpha - \vec u\, \rangle\ =\ \frac{u^2}{2}[\cancel{\rho\vec u} - \cancel{\rho\vec u}]\ =\ 0\ \ . \]
...
\[ \frac{1}{2} \sum_\alpha m_\alpha \nabla\, \cdot\, ( n_\alpha\, \langle v^2 \vec v \, \rangle_\alpha )\ =\ \nabla \cdot (\frac{1}{2}\rho u^2 \vec u + \frac{3}{2}\, p\, \vec u\, + \mathbf{P} : \vec u\, + \mathbf{Q}\, ) \]


Otteniamo così l'\underline{equazione dell'energia per singolo fluido} (in caso \emph{collision-less}):
...
%
%
\subsection{Ancora sull'equazione dell'energia}
...
\[ \frac{\partial}{\partial\, t} \left(\frac{1}{2}\,\rho\, u^2\, +\, \frac{3}{2}\, p\right)\ + \ \vec\nabla\cdot\left[\frac{1}{2}\,\rho\, u^2\,\vec u\, +\, \color{blue}\frac{3}{2}\, p\, \vec u\, +\, \mathbf{P}\cdot\vec u\, \color{black} +\, \vec q \right]  \ =\ \vec j\cdot \vec E \]
...
\begin{equation}
	...
	\frac{\partial}{\partial\, t} \left(\frac{1}{2}\,\rho\, u^2\, +\, \frac{3}{2}\, p\right)\ + \ \vec\nabla\cdot\left[  \underset{u^2\,(\rho\vec u)}{\underbrace{\frac{1}{2}\,\rho\, u^2\,\vec u\,}} +\, \color{blue}\frac{5}{2}\, p\, \vec u\,\color{black} +\, \vec q \right]  \ =\ \vec j\cdot \vec E
\end{equation}

Sviluppando eq.(\ref{Energia singolo fluido v2}):
...
Vogliamo ora sottrarvi l'equazione del moto in eq.(\ref{moto MHD}), moltiplicata per $\vec u$, che riportiamo qui di seguito,
...
%
\vspace{0.5cm}
\begin{approfondimento}{Altre definizioni importanti}
...
%
\newpage
\section{Riassumendo il framework di MHD ideale}
