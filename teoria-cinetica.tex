...
&=\ \int v_1 \cdot \frac{\partial F^{(n)}}{\partial \vec x_1 } dx_2 ...dx_n dv_2 ...dv_n + \cancel{\int \sum^N_{i=2} \vec v_i \cdot \frac{\partial F^{(n)}}{\partial \vec x_i} dx_2 ...dx_n dv_2 ...dv_n} \ =\\ 
...
&\ \ \ \ \ \ \ \ ...\ +\ \underset{\vec a_i F^{(n)} - \int \frac{\partial \vec a_i}{\partial\vec v_i} F^(n)dx_2\dots dx_n dv_2\dots dv_n }{\underbrace{\sum^N_{i=2}\cancel{\int \vec a_i \cdot \frac{\partial F^{(n)}}{\partial \vec v_i} dx_2 ...dv_n}}} \ \ =\ \ \vec a_1 \cdot \frac{\partial F^{(1)}}{\partial \vec v_1} \ \ \ \ .
\end{split}\]



Per esplicitarla $\vec a_i\, =\, \frac{q}{m}(\vec E\,+\, \vec v_i\cdot \vec B\, /\, c )$, si arriva così, nel limite in cui è possibile ignorare lo scattering di particelle, all'equazione cardine della descrizione statistica dei plasmi, l'\underline{equazione di Vlasov}:
...
Quest'ultima trovata è - di fatto - un'\underline{equazione del moto}.
