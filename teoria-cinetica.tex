...
Trattare un plasma con un \underline{modello \emph{many-body}} ci porterebbe ad un problema irrisolvibile, per questo introduciamo ora un framework statistico che ci permetterà di ricavare l'equazione di Vlasov e da questa di ricollegarci alle equazioni fluide attraverso i suoi momenti.

Continueremo a supporre di essere in un \underline{regime non-collisionale}, le particelle di ogni specie saranno sottoposte a campi elettrici e magnetici collettivi, l'energia cinetica sarà da considerarsi molto maggiore dell'energia d'interazione in modo che la generica carica sia 'statisticamente indipendente' dalle altre. 
...
Come prima cosa si introduce la \underline{funzione di distribuzione},
...
dove \textbf{$P_{12}$} è detta \textcolor{blue}{funzione di correlazione} (cfr. gerarchia di Bogoliubov-Born-Green-Kirkwood-Yvon in Fisica Statistica). 
...
\begin{teorema}{Teorema di Liouville}
	 SSiccome sarà utile in seguito, richiamiamo il teorema di Liouville: data la nostra funzione di distribuzione F che descrive il plasma ad ogni tempo, questa occuperà un punto dello spazio delle fasi 2n-dimensionale, tracciando nel tempo una traiettoria di fase; secondo il teorema la derivata temporale totale della densità degli stati sarà nulla.
...
%
\vspace{0.5cm}
\section{L'equazione di Vlasov}

Dal teorema di Liouville in eq.(\ref{Liouville}) si giunge ad una delle più importanti equazioni della fisica dei plasmi, l'equazione di Vlasov: integrando su tutte le particelle da 2 ad N vediamo che il primo termine di eq.(\ref{Liouville}) è legato alla $F^{(1)}$.

Andiamo ad integrare separatamente i termini di eq.(\ref{Liouville}):

\[ \int\ \frac{\partial F^{(n)}}{\partial t}\ dx_2 ...dx_n dv_2 ...dv_n\ =\ \frac{\partial}{\partial t }\ \left( \int F^{(n)}\ dx_2 ...dx_n dv_2 ...dv_n\right)\ =\ \frac{\partial F^{(1)}}{\partial t}\]

Mentre per il secondo e terzo termine di eq.(\ref{Liouville}):

\[\begin{split} 
\int\sum^N_{i} \vec v_1 \cdot \frac{\partial F^{(n)}}{\partial \vec x_1} dx_2 ...dx_n dv_2 ...dv_n\ 
&=\ \int v_1 \cdot \frac{\partial F^{(n)}}{\partial \vec x_1 } dx_2 ...dx_n dv_2 ...dv_n + 
\overset{\text{per }i=2\ \ \ \int [\int \vec v_2 \cdot(\cancel{\int \frac{\partial f^{(n)}}{\partial x_2}dx_2)}dv_2]d\Gamma_{3...n}}{\overbrace{\cancel{\int \sum^N_{i=2} \vec v_i \cdot \frac{\partial F^{(n)}}{\partial \vec x_i} dx_2 ...dx_n dv_2 ...dv_n}}} \ =\\ 
&= \vec v_1 \cdot \frac{\partial}{\partial \vec x_1} \int F^{(n)} dx_2 ...dx_n dv_2 ...dv_n \ =\ \vec v_1 \cdot \frac{\partial F^{(1)}}{\partial \vec x_1}\ \ \ \ ,
\end{split}\]

\[\begin{split} \int \sum^N_{i}\vec a_i \cdot \frac{\partial F^{(n)}}{\partial \vec v_i} dx_2 ...dx_n dv_2 ...dv_n\ 
&=\ \vec a_1\cdot \frac{\partial}{\partial \vec v_1} \int F^{(n)} dx_2 ... dv_n\ +\ ... \\
&\ \ \ \ \ \ \ \ ...\ +\ \underset{\vec a_i F^{(n)}|_.^. - \int \frac{\partial \vec a_i}{\partial\vec v_i} F^{(n)}dx_2\dots dx_n dv_2\dots dv_n }{\underbrace{\sum^N_{i=2}\cancel{\int \vec a_i \cdot \frac{\partial F^{(n)} }{\partial \vec v_i} dx_2 ...dv_n}}} \ 
\ =\ \ \vec a_1 \cdot \frac{\partial F^{(1)}}{\partial \vec v_1} \ \ \ \ .
\end{split}\]


Esplicitiamo l'accelerazione $\vec a_i\, =\, \frac{q}{m}(\vec E\,+\, \vec v_i\cdot \vec B\, /\, c )$, si arriva così, nel limite in cui è possibile ignorare lo scattering di particelle, all'equazione cardine della descrizione statistica dei plasmi, l'\underline{equazione di Vlasov}:

\begin{equation}\label{VLASOV}
	\boxed{\ \frac{\partial F^{(1)}_\alpha}{\partial t}\ +\ \vec v_1 \cdot \frac{\partial F^{(1)}_\alpha}{\partial \vec x_1}\ +\ \frac{q_\alpha}{m_\alpha}\left( \vec E + \frac{\vec v \times \vec B}{c}\right)\cdot\frac{\partial F^{(1)}_\alpha}{\partial \vec v_1}\ =\ 0\ }
\end{equation}

Ma in questa forma abbiamo tenuto conto unicamente dell'accelerazione dovuta ai campi, per tener conto di interazioni fra particelle aggiungiamo un termine di interazione binaria $\vec a_{ij}$ (così che $\vec a_i\, =\, \frac{q}{m}(\vec E\,+\, \vec v_i\cdot \vec B\, /\, c)\, +\, \vec{a}_{ij}$ ) a quest'accelerazione e integriamo anch'esso:
...
Questa è conosciuta come \underline{equazione di Boltzmann}, può essere scritta per ciascuna specie ma per determinarne il risultato sarà necessario esplicitare $\vec E$, $\vec B$ ed il termine di scattering $\mathcal{C}\sim (\partial f_S/\partial t)_{coll.}$.
...
\[n_\alpha \ =\ \int f_\alpha\ d\textbf{v} \ \ , \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \  \ \ \ \ \ \ \ n_\alpha \vec u_\alpha\ =\ \int f_\alpha \vec v_\alpha \ d\textbf{v}\ \ .  \]
Questi sono momenti di ordine zero ed uno della funzione di distribuzione $f_\alpha$ [quindi il momento di ordine n della funzione di distribuzione è definito come $\int v^n\, [\text{f. di distr.}]\, dv$ mentre ora andremo a calcolare i momenti dell'equazione di Vlasov come: $\int v^n\, [\text{eq. di Vlasov}]\, dv$ ].
...
Rimettendo assieme questi 3 membri si ottiene dunque infine l'\underline{equazione di continuità} per la specie $\alpha$, dove rientrano i momenti di ordine zero ed uno di $f_\alpha$ (questo è un problema, è da qui che si capisce subito che serve una chiusura):
...
\[\begin{split}\frac{\partial}{\partial \vec v_\alpha} \left[ \vec v_\alpha \vec D f_\alpha\right] \ 
&=\  f_\alpha \vec D \frac{\partial \vec v_\alpha }{\partial\vec v_\alpha} \,+\, \vec v_\alpha \frac{\partial(f_\alpha \vec D)}{\partial\vec v_\alpha}\ =\ \\ 
&=\ f_\alpha \vec D \frac{\partial \vec v_\alpha }{\partial\vec v_\alpha}\, +\, \vec v_\alpha D\cdot \frac{\partial f_\alpha }{\partial\vec v_\alpha} \quad ,\end{split}\]

\[ \int \vec v_\alpha\, \vec{D}\, \frac{\partial f_\alpha}{\partial \vec v_\alpha}\, d\vec v_\alpha\ =\ -\int \vec D\, f_\alpha\, d\vec{v_\alpha}\ =\ -\,n_{\alpha}\vec{D}\ =\ \, -\, n_{\alpha}\vec{E} \,-\, n_\alpha \frac{\vec u_\alpha \times \vec B}{c}\ ,\]
...
Così infine da Vlasov si arriva all'\underline{equazione del moto} :

\begin{equation} 
	\label{moto 1}
	\boxed{\frac{\partial (n_\alpha \vec u_\alpha)}{\partial t}\ +\ n_\alpha\, \nabla <\vec v \vec v>_\alpha\ -\ n_\alpha \frac{q_\alpha}{m_\alpha} \left(\vec E + \frac{\vec u_\alpha \times \vec B}{c}\right)\ =\ 0 } 
\end{equation}

Per il calcolo di $<\vec v \vec v>_\alpha$ è utile scomporre la velocità in $\vec v = \vec u + \vec w$, dove $\vec u = <\vec v>$ è la velocità media mentre $\vec w$ è la velocità ('peculiare') della singola particella che si discosta dalla media, che verifica $<\vec w>=0$.
...
Quest'ultima trovata è - di fatto - un'altra forma dell'equazione del moto in eq.\ref{moto}, ma ci permetterà ora di introdurre il tensore di pressione.

\vspace{0.5cm}
\begin{approfondimento}{Definizioni}
	La velocità introdotta sopra $\boxed{\vec w = \vec v - \vec u}\ \ (= \vec v - \langle \vec v \rangle)$ è detta \underline{velocità peculiare}.
	Possiamo definire anche un \underline{tensore di pressione:}
	\[\mathbf{P}\ =\ m\int\, (\vec v - \vec u)\,(\vec v - \vec u)\, f(\vec x, \vec v, t)\, d\vec v\ =\ m \,n_\alpha\, \langle \vec w \vec w\rangle\ \ \ \ ,\]
	da cui si ottiene una versione più usata dell'equazione del moto in eq.\ref{moto}:
	...
	\begin{equation}
		\label{moto eq. cinetica con pressione}
	...
	che in un gas ideale - per il quale la pressione è legata alla temperatura da $p_\alpha=n_\alpha\, T_\alpha$ - ci porta alla temperatura in funzione di termini cinetici $T=\frac{1}{3}m_\alpha |w|^2_\alpha$ (più che una temperatura una stima dell'energia cinetica per specie).
\end{approfondimento}
...
Come si vede evidenziato in blu abbiamo definito tre grandezze: nel primo termine si è definita una sorta di energia media (più un'\underline{energia cinetica collettiva locale}) $\boxed{\varepsilon = \displaystyle\int \frac{1}{2} m v^2 f d\vec v=\frac{1}{2}mn\langle v^2\rangle}$; nel secondo termine si è definito il \underline{flusso di energia convettiva} $\boxed{\vec Q= \frac{1}{2}m\int v^2 \vec v f d\vec v = \frac{1}{2}mn\langle v^2 \vec v\rangle}$; nel terzo termine invece riincontriamo una definizione di \underline{corrente di particelle} ($\vec J=nq\vec u$).
...
\[\varepsilon\ =\ \frac{1}{2} m n \langle u^2\rangle + \frac{1}{2} m n \langle w^2\rangle\ =\ \frac{1}{2} m u^2 + \frac{3}{2} n T\ \ ,\] 

per il flusso di energia convettiva invece 
\[Q\ =\ \frac{1}{2} m u^2 (n\vec u) + \frac{3}{2} n T\vec u +\frac{1}{2} m n (2\vec u\cdot \langle \vec w \vec w \rangle) +\frac{1}{2}mn\langle w^2\vec w\rangle\ \ ,\]

dove il primo termine di questi è un flusso di energia, il secondo un flusso di calore $\vec q = \frac{3}{2} n T\vec u$, il terzo è invece dovuto alle forze di pressione ( $P\cdot u = \frac{1}{2} mn(2\vec u\cdot \langle \vec w \vec w\rangle )$ ) ed il quarto ad un lavoro di pressione macroscopico.