...
\begin{approfondimento}{Elemento fluido}
	...
	Il cubetto $\Delta V$, detto \textit{elemento fluido}, ha la caratteristica che il tempo con cui una molecola ne fuoriesce è molto maggiore del tempo caratteristico di evoluzione del sistema (dato l'altissimo numero di collisioni/rateo di collisioni nei fluidi).
...
\[\underset{\text{flusso di massa}}{\underbrace{\int_A\rho\vec v\hspace{1mm} \,d\vec A}} \ =\ -\, \underset{\text{variazione densità nel volume}}{\underbrace{\frac{\partial}{\partial t}\int_V\,\rho\, dV}}\ \ \ \ \ \ \Rightarrow\ \ \ \ \ \
...
\section{Equazioni del moto (tre componenti)}
...
\section{Evoluzione dell'elemento fluido} 
...
Poiché  $\displaystyle\frac{\partial \vec x}{\partial t} = \frac{\partial \vec \xi}{\partial t} \quad \Rightarrow \qquad \vec v\, =\, \vec u$
...
\section{Collisioni e diffusione del calore}
...
\[ \text{energia interna specifica: }\quad U\ =\ \rho\, c_v\, T \ \ \ \ ,\]
\[ \text{trasporto di calore: }\quad \vec{q} = -k \ \vec\nabla T\ \ \ \ ,\]

dove $\vec{q}$ è la \underline{densità di corrente termica}, $k$ la \underline{conducibilità} ($[K]=W/(m\,K\,)$) e $c_v$ la \underline{calore specifico a volume costante} ($[c_v]=J/(Kg\,K\,)$)

L'energia interna varia a seconda: dello scambio di calore, della compressione o dal calore generato internamente:
\[
\underset{\text{variazione di energia interna}}{\underbrace{\frac{\partial}{\partial t}\int U \, dV}}\ =\ -\, \underset{\text{flusso di calore}}{\underbrace{\int (\vec q \cdot \hat n)\, dS}} \ \ \ \ .
\]
...
avendo definito la \underline{diffusività} $\boxed{D = k\, /\, \rho\, c_v}$, che ha dimensioni $[D]=m^2 s^{-1}$.
...
\[\nu_c\to\infty\quad \implies\quad  D\to 0\]
...
Quest'equazione descrive l'\underline{evoluzione della vorticità}.
...
%
\vspace{0.5cm}
\section{Regime a due fluidi} \label{sezione su due fluidi, lineariz. e onde di plasma}
...
Prendiamo invece dall'equazione del moto, trascurando le collisioni ed il termine gravitazionale, andiamo a linearizzare l'equazione tenendo i termini di ordine più basso, ponendo $\vec u_0 = 0$ e la sua derivata $\partial_t u_0 = 0$ :
...
Definita $c_s^2 = \gamma \displaystyle \frac{P_0}{\rho_0}$ la \textbf{velocità del suono}, possiamo ottenere infine l'\underline{equazione del moto linearizzata}: 

\[\Rightarrow \ \ \ \  \boxed{\ \displaystyle \rho_0 \frac{\partial}{\partial t} (\delta u)\,=\, c_s^2\, \frac{\partial }{\partial x} (\delta \rho)\ }\ \ . \] 
...
\[ \begin{cases}
	...
	-\,i\omega\, \rho_0\, \delta u\ =\ -\,i k\, \delta P\ ,\\ \\
...
Conclusione: l'onda sonora in un gas è non dispersivo, con relazione di dispersione costante (ogni armonica si propaga alla stessa velocità, ovvero $\omega / k =$ costante). 
...
Linearizziamo anche le densità numeriche di elettroni e ioni $n_{e/i} = \displaystyle\frac{N_{e,i}}{V} = n_{0,\ e/i} + \delta n_{e/i}$ ($n_e\sim n_i$) e consideriamo gli ioni come immobili, mentre per gli elettroni $\vec u_e = \vec u_0 + \vec{\delta u}$ con $\langle \vec u_0 \rangle = 0$.

Questa volta vogliamo studiare il comportamento aggiungendo un campo $\vec E = \vec E_0 + \vec{\delta E}$ con $\vec B_0 = 0$ e per il campo elettrico usiamo assieme l'equazione di Poisson espressa come $\vec \nabla \cdot \vec E = 4\pi(n_{0, i}-n_{0, e}-\delta n_e)$ e la ben nota $\vec E = -\vec \nabla \phi$:
...
Da dalle condizioni in cui ci troviamo ora ci rendiamo conto manchi una chiusura, perché abbiamo come incognite $\delta n_e,\, \delta u,\, \delta P,\, \delta \phi$ ma solamente 3 equazioni nel sistema, come fare?
\begin{enumerate}
	\item \underline{Politropica}: lega fra loro pressione e densità, come se fosse un gas, dopotutto siamo in un modello fluido.
...
\[\begin{cases}
	...
	\text{da Eulero: }-i\omega m_en_0\delta u= \cancelto{\sim 0}{-ik\,\delta P}\ +\ ik\,en_0\delta \phi\ , \\
...
\emph{[Si è isolato il termine $\delta u$ nelle prime due equazioni del sistema e nella seconda si è inserita una forma di $\delta \phi$ trovata dalla terza, così da avere $\delta u=\frac{\omega}{kn_0}=\frac{ke}{m_e \omega}\frac{4\pi e}{k^2}\delta n_e$, da qui la relazione trovata segue immediatamente]}.
...
Si faccia attenzione al fatto che questa relazione è dispersiva, non è che se non compare $k$ nell'espressione di $\omega$ allora non c'è dispersione, perchè non c'è dispersione quando $\omega/k=cost$. è vero, però che in questo caso la \underline{velocità di gruppo} è nulla, $\boxed{v_g\equiv \frac{d\omega}{d k}}=0$.
...
Nel ricavare la frequenza di plasma abbiamo supposto che la \underline{velocità termica degli elettroni} $v_{th, e}$ fosse trascurabile rispetto alla \underline{velocità di fase} 
...
\[-i\omega m_en_0\delta u\ =\ iken_0\delta \phi\, -\, ik\,\color{blue}T_{0,e} \delta n_e\color{black}\ .\]
...
\vspace{0.3cm}
Introduciamo anche la \underline{lunghezza d'onda caratteristica} $\lambda\sim 1/k$ e confrontiamo $\dfrac{\omega^2}{k^2}$ con $v_{th,e}^2$:

\[\dfrac{\omega^2}{k^2}\ =\ \frac{4\pi n_0 e^2}{m_e}\lambda^2\ \sim\ v_{th,e}^2\ \ ,\] 

\[\lambda^2\ >\ \frac{v_{th,e}^2}{\omega_p^2}\ =\ \frac{T}{4\pi n_0e^2}\ \equiv \lambda_D\ \ .\] 
...
L'espressione a membro destro dell'ultima disuguaglianza è definita come lunghezza caratteristica, è detta \underline{lunghezza di Debye}, $\frac{T}{4\pi n_0e^2}\equiv \lambda_D$. Nello spazio può venire misurata. 
...
Abbiamo già introdotto in questo capitolo l'idea di usare $P_e=cn_e^\gamma$ (espressa in precedenza anche come $P=c\rho^\gamma$ o $\frac{d(P\rho^{-\gamma})}{dt}=0$) come chiusura alle nostre equazioni, ora torniamo a farlo per ricavare la correzione termica alla relazione di dispersione ricavata per il plasma freddo:
...
\[\begin{cases}
	\text{dalla politropica: }\quad\delta P=\gamma T_{0,e}\delta n_e \\
	\text{dalla continuità: }\quad\displaystyle\frac{\omega}{k}\delta n = n_0\delta u\ ,\\
	\text{dal moto: }\quad-i\omega m_en_0\delta u= en_0\delta E - ik\delta P , \\
	\text{da Poisson: }\quad\delta \phi =-\displaystyle\frac{4\pi e}{k^2}\delta n_e\ .
\end{cases}\]
...
Riconosciamo le definizioni date precedentemente e troviamo la nostra relazione di dispersione, chiamata in letteratura anche \underline{Bohm-Gross dispersion relation}:

\begin{equation}
	\boxed{\ \omega^2=\omega^2_p+\gamma k^2v_{th,e}\ }
\end{equation}

\[\omega^2\ =\ \omega_p^2\, \left(1+\gamma\frac{k^2v_{th,e}}{\omega_p^2}\right)
\ =\ \omega_p^2\,\left(1+\gamma k^2\lambda_D^2\right)\] 

\begin{figure}[H]
	\centering
	\includegraphics[width=9cm]{immagini/langmuir.jpeg}
	\caption{Rappresentazione approssimativa della dispensione $\omega/\omega_P=(1+\gamma k^2 \lambda_D^2)$. Non vi sono onde di Langmuir per $\omega < \omega_P$.\\ Per grandi lunghezze d'onda (bassi k) o basse temperature si osservano le oscillazioni di plasma, o di Langmuir, causate da una velocità di fase $\omega/k$ che può essere arbitrariamente grande ma con una velocità di gruppo $\partial \omega/\partial k$ che va a zero, non vi è propagazione.\\ Per piccole lunghezze d'onda sia la velocità di fase che di gruppo convergono a $\sqrt{3}v_{th,e}$ e l'onda di plasma si propaga come un'onda sonora.}
	\label{Langmuir dispersion}
\end{figure}
\end{multicols}
...
\[n_i\ =\ n_0\, e^{+e\phi/T}\ \ ,\, \quad \quad \quad n_e\ =\ n_0\, e^{-e\phi/T}\ \ .\] 
...
Inseriamo queste densità delle due specie in Poisson per avere un'equazione dalla quale definiamo la \underline{lunghezza di Debye} $\lambda_D = \sqrt{\displaystyle\frac{T}{8\pi e^2 n_0}}$ :
...
Usando queste due forme rispetto ad $u$ per le derivate possiamo riscrivere il laplaciano e tutta l'eq.\ref{Poisson} per avere:
...
%
\vspace{0.5cm}
\section{Onda e.m. che investe un plasma}
...
\[ \vec\nabla\times\vec E\ =\ \frac{i\omega}{c}\, \vec B\quad \quad \Rightarrow 
\quad \quad 
\vec B\ =\ -\,\frac{i c}{\omega}\,\vec \nabla\times \vec E \]
...
Sappiamo poi da una identità che $\vec k\times(\vec k\times \vec E) = (\vec k\cdot \vec E)\vec k - k^2\vec E$, ma se \underline{ $\vec k\perp \vec E$ } , allora $\vec k\cdot \vec E = 0$:

\begin{equation}
	\label{dispersione longitudinale}
	-\cancelto{=0}{(\vec k\cdot \vec E)\vec k}\,+\,k^2\vec E\ =\ \frac{\omega^2}{c^2} \,\left(1-\frac{\omega^2_P}{\omega^2} \right)\, \vec E\quad \quad \Rightarrow \quad \quad \boxed{\ \omega^2\ =\  k^2 c^2\,+\, \omega^2_P \ }
\end{equation}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=8cm]{immagini/HF-EM-wave.png}
	\caption{Relazione di dispersione di onda e.m. trasversa ($\vec k \perp \vec E$)}
	\label{HF-EM wave dispersion - unmagnetized}
\end{figure}
...
\begin{itemize}
	...
	%
	\item  nel caso contrario, $\omega \ll \omega_P$, l'onda ha una relazione di dispersione 
	\[k^2 = (\omega^2 -\omega^2_P)/c^2 \approx -\omega^2_P/c^2\ \ ,\]
	 questo vuol dire che k è complesso ($k\approx i\, \omega_P/c$). Questo cosa implica? $\exp(i(i\frac{\omega_P}{c})x) = \exp(-\omega_P x / c)$, l'onda non ha più un esponente immaginario ma è \underline{evanescente} perché reale e decrescente. 
	Dal caso di onda evanescente definiziamo la lunghezza caratteristica di smorzatura dell'esponenziale e quindi dell'onda che penetra nel plasma, la \underline{electron skin depth}: \[\boxed{\, d_e \equiv c\, /\, \omega_P\, =\, \sqrt{m_e/4\pi e^2 n_{0,e} }\, c\ }\ \ .\]
\end{itemize}

Si nota che nel caso di onda con \underline{ $\vec k\parallel \vec E$ } invece la relazione di dispersione di riduce ad 
\[\boxed{\, \omega^2\  =\ \omega_P^2\, }\]
...
\begin{approfondimento}{velocità di gruppo e di fase}
	...
	\[ \frac{\omega}{k}\ =\ \frac{\sqrt{k^2\omega^2 + \omega^2_P}}{k}\ =\ \sqrt{1+\frac{\omega_P^2}{k^2 c^2}} \, \, c\ >\ c\ \ .\]
...
\begin{approfondimento}{Sulle grandezze caratteristiche dei plasmi}
	...
	Si vede anche, dalle definizioni $\lambda_D\ =\ v_{th}\, /\,\omega_P$
...
\[ \Rightarrow \qquad \frac{V}{T} \,\sim\, \frac{e^2}{n^{1/3}\, T}\, \sim\, \frac{\nu_{coll.}}{\omega_P}\qquad \Rightarrow \qquad \frac{V}{T}\ll\ 1\]
...
\[ \text{da definizione:}\quad \quad \lambda_D\ =\ \sqrt{\frac{k_BT}{4\,\pi\,n\, e^2}} \]

Come calcolare il numero di particelle cariche $N_D$ contenute nella sfera?

\[ N_D\ =\ n\,\lambda_D^3\ =\ n\,\left(\sqrt{\frac{k_B\, T^3}{n\, e^2}}\right)^3\ =\ \frac{(k_B T)^{3/2}}{(4\pi e^2)^{3/2}\, n^{1/2}}\]

Una prima stima del numero di cariche contenute nella sfera di Debye è $n\, \lambda_D^3$ e - notando la dipendenza $n\, \lambda_D^3 \approx (T\, /\, V)^{3/2}\ll 1$ - si trova che un plasma rispetta la condizione: $\boxed{N_D\sim n \, \lambda_D^3\gg 1}$.
...
%
\vspace{0.5cm}
\section{Introduzione a teoremi fondamentali nella fisica dei plasmi} 
...
\begin{teorema}{Alfvén}{}
	...
	Abbiamo così dimostrato il teorema per cui $\boxed{\frac{d\Phi}{dt} = 0}$ .
\end{teorema}
