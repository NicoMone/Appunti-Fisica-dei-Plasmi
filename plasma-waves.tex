...
La scelta del segno + porta ad una magnetosonica simile al modo di Alfvén ma comprimibile, $\omega_f\equiv (\omega^+/k)^2=C_A$. Diversamente la scelta del segno - ci porta a definire $\omega_s\equiv (\omega^+/k)^2=c_s \cos\theta$ in cui compare solamente la velocità del suono.
...
La teoria cinetica non avrebbe previsto dei fenomeni di riconnessione mentre l'MHD resistiva lo permette ed avviene una rottura delle linee di campo su scle molto più piccole di quelle MHD, generando però ripercussioni globali sulla dinamica.
