...
\begin{enumerate}
	...
	Questa può essere vista come una sorta di velocità del suono, infatti queste sono le tipiche onde ionacustic. Si vede che l'inerzia è data dagli ioni nel senso che compare $m_I$ mentre la risposta termica è data da entrambi, anche se quando $T_I\sim T_e$ tipicamente avviene il Dumping di landau e l'onda non si propaga (a meno che non si faccia una forzatura). 
	Le onde che sopravvivono sono quelle, invece, con $T_e\gg T_i$ [\textit{"Se poi ho un forzaggio sono fuori dal regime lineare perché cambio la funzione di distribuaione e cambia anche la risposta dielettrica"}, boh] 
...
Il dumping di Landau è spesso chiamato "smorzamento", ma non è un fenomeno dissipativo. Ecco le sue proprietà chiave:
\begin{itemize}
	...
	\item \underline{È un trasferimento di energia} dall'onda alle particelle (o viceversa);
...
\begin{itemize}
	...
	\item \(\tau_L= \displaystyle\frac{1}{k^3\lambda_D^3}e^{-(\frac{3}{2}+\frac{1}{2k^2\lambda_D^2})}\): tempo caratteristico del dumping di Landau;
