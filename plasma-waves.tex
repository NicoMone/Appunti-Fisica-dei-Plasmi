...
Assumendo come soluzioni delle onde piane per pressioni e velocità ($\delta \rho (x, t) = \hat \rho e^{i(kx-\omega t)}$ e $\delta u (x, t) = \hat u e^{i(kx-\omega t)}$) e facendo una trasformata di Fourier delle equazioni del sistema ($\partial_x\rightarrow ik$, $\partial_t\rightarrow -i\omega$) si è trovata la relazione di dispersione:
\begin{equation}
	\label{acoustic wave}
	\omega^2\ =\ k^2\, c_s^2
\end{equation}
...
Onde elettrostatiche di tipo acustico/sonoro, si possono ricavare da vlasov ma noi non lo facciamo, le ricaviamo con il modello a due fluidi (cfr. sezione 10.6 Pucella-Segre, "\emph{Onde elettrostatiche in plasmi caldi, senza $B_0$}", con un procedimento del tutto uguale a quello che seguiremo di seguito si possono trovare le stesse onde di plasma che abbiamo già studiato e riassunto qui, per queste cfr. sezione 10.5 "\emph{Onde in un plasma freddo, senza $B_0$}"). 
...
\paragraph{Equazioni di base e ipotesi: } 

Consideriamo le grandezze all'equilibrio con perturbazioni di piccola grandezza:

\[n_\alpha=n_{0\alpha}+n_{1\alpha}\hspace{1cm}P_\alpha=P_{0\alpha}+P_{1\alpha}\hspace{1cm}\text{con }\alpha=I,e\]
...
\begin{cases}
	...
	m_\alpha n_\alpha \left(\frac{\partial \vec u_\alpha}{\partial t} + \vec u_\alpha \cdot \vec \nabla \vec u_\alpha \right)\ =\ -\vec \nabla P_\alpha\, +\, n_\alpha q_\alpha\,\left(\vec E + \frac{1}{c}\vec u_\alpha \times \vec B\right)\ \ ,\\
	\frac{D}{Dt} \left(P_\alpha n_\alpha^{-\gamma_\alpha}\right)\ =\ 0\ \ .	
\end{cases}
\]

Andiamo a linearizzare come già fatto precedentemente ma per la politropica in particolare osserviamo che il termine $\vec u_1\cdot \vec \nabla$ della derivata materiale è di ordine successivo per cui rimane $\frac{\partial p_\alpha}{\partial t} n_\alpha^{-\gamma_\alpha} + p_\alpha\frac{\partial (n_\alpha^{-\gamma_\alpha)}}{\partial t} = 0$:
...
\[\begin{aligned}
	...
	& m_\alpha\, n_{0\alpha}\,\partial_t\vec u_{1\alpha}\ =\ -\nabla P_{1\alpha}\,+\,n_{0\alpha}q_{\alpha}\,(\vec E+\cancelto{\sim 0}{\frac{\vec u_{1\alpha}\times\vec b}{c}})\\
...
il termine $\vec u_1\times\vec b$ è del second'ordine ed è pertanto per noi nullo, significa che per la nostra onda che è polarizzata longitudinalmente $\vec k\parallel \vec E$, la seconda equazione ci conduce alla condizione:

\[\vec E_1\ \parallel\ \vec u_1\ \parallel\ \vec k\ \ \ .\]
...
\[\begin{cases}
	...
	\\
	P_{1\alpha}\ =\ \gamma_\alpha P_{0\alpha}\, \frac{n_{1\alpha} }{n_{0\alpha}}\,    
...
\begin{equation}
	\label{permittività}
	\epsilon\ =\ 1+\frac{4\pi i}{\omega}\sigma\ =\ 1\,-\,\sum_\alpha \frac{ \omega^2_{P,\, \alpha} }{ \omega^2-k^2v^2_{0,\alpha} } \ \ . 
\end{equation}
...
\begin{cases}
	...
	\omega^2_{IA}\ \simeq\ \frac{AB}{4}\ =\ \frac{C}{A}\ =\ \frac{ k^2(\omega^2_{p,\,I}v^2_{0,\,e}+\omega^2_{p,\,e}v^2_{0,\,I} + k^2 v^2_{0,\,e} v^2_{0,\,I} )}{k^2(v^2_{0,\,e}+v^2_{0,\,I})+ \omega^2_{p,\,e} + \omega^2_{p,\,I}}
\end{cases}
\]
...
\begin{equation} \label{ionoacustiche}
	\boxed{\frac{\omega^2_\text{IA}}{k^2}\ =\ v_{0I}^2\ \frac{k^2\,+\, \displaystyle\frac{1}{\gamma_e \lambda_\text{De}^2}\,+\,\frac{1}{\gamma_I \lambda_\text{DI}^2}}{k^2\,+\,\displaystyle\frac{1}{\gamma_e \lambda_\text{De}^2}}}\ \ \  .
\end{equation}
...
\begin{enumerate}
	\item il \underline{caso classico} è quello per cui $k^2\,\ll\, \lambda_{\text{D, e}}^{-2}\,\ll\, \lambda_{\text{D, I}}^{-2}$ :
	...
	Nella relazione di eq. \ref{ionoacustiche} possiamo trascurare $k^2$ al denominatore, ma anche al numeratore visto che conta molto meno di $1/\lambda^2_{D,e}$. Per questo rimane la seguente relazione che semplifichiamo osservando che $\frac{\gamma_e \lambda^2_{D,e}}{\gamma_I \lambda^2_{D,I}} = \frac{m_e v^2_{0,e}}{m_I v^2_{0,I}}$:
	
	\[\begin{aligned}
		\frac{\omega^2}{k^2}\ &=\ v^2_{0,I}\, \left[\frac{(\gamma_e \lambda^2_{D,e})^{-1} + (\gamma_I \lambda^2_{D,I})^{-1}}{(\gamma_e \lambda^2_{D,e})^{-1}}\right]\ =\ v^2_{0,I}\, \left[1\, +\, \frac{(\gamma_I \lambda^2_{D,I})^{-1}}{(\gamma_e \lambda^2_{D,e})^{-1}}\right]\ =\\
		&=\ v^2_{0,I}\, \left[1\, +\, \frac{m_e v^2_{0,e}}{m_I v^2_{0,I}} \right]\ =\ v^2_{0,e} \frac{m_e}{m_I} + v^2_{0,I}\ \simeq\ c^2_s
	\end{aligned}\] 
	
	Abbiamo trovato le \underline{onde ionico-acustiche} usufruendo della definizione di \underline{velocità ionico-acustica},
	
	\[c^2_s\ =\ \gamma\frac{P_0}{\rho_0}\ =\ \frac{\gamma_e T_e n_{0,e} + \gamma_I T_I n_{0,I}}{m_e n_{0,e} + m_I n_{0,I}}\ \simeq\ v^2_{0,e} \frac{m_e}{m_I} + v^2_{0,I}\ \ .\]
	
	%
	%
	\item  Abbiamo poi il caso delle \underline{oscillazioni di plasma ioniche}:
	...
	Data la prima disuguaglianza, il $k^2$ della relazione di dispersione può essere non considerato al numeratore di eq.\ref{ionoacustiche}, così come il termine $1/\lambda^2_{D,e}$ rispetto a $1/\lambda^2_{D,e}$. Al denominatore invece domina $k^2$ per cui rimane:
	
	\[ \frac{\omega^2}{k^2}\ =\ \frac{v^2{0,I}}{k^2 \gamma_I \lambda^2_{D,I} }\ =\ \frac{\omega^2_{P,I}}{k^2}\ \ . \] 
	
	Torna la definizione di frequenza di plasma ionica $\omega^2_{P,I}=\frac{4\pi\, e^2\, n_{0, I}}{m_I}$, usata insieme a $v^2_{0\alpha}=\frac{\gamma_\alpha P_{0\alpha}}{m_\alpha n_{0\alpha}}$ ed a $\gamma^2_{D,\alpha}\sim \frac{v^2_{0,\alpha}}{\omega^2_{P,\alpha}}$
	...
	\[\omega^2\ =\ k^2 v^2_{0,I}\, \left(1\, +\, \frac{1}{k^2 \gamma_I \lambda^2_{D,I}}\right)
	\ =\ \omega_{pI}^2\ +\ k^2\,v_{0I}^2\] 
	
	In particolare considerando $k^2\gg \lambda^{-2}_{D,I}$ rimane solamente $\omega^2=k^2 v^2_{0,I}$.
\end{enumerate} 
...
\begin{figure}[H]
	\centering
	\label{ionoacustiche dispersione}
	\includegraphics[width=7.5cm]{immagini/dispersioneionacoustic.jpg}
	\caption{Raffigurazione delle relazioni di dispersione trovate, in particolare in basso si osservano le onde ion-acoustic con i 3 regimi discussi.}
\end{figure}
...
Le \textbf{onde di Alfvén} sono onde \underline{non dispersive}, inoltre non sono \underline{compressive} ($\vec k \cdot \vec u\, =\, 0$), quindi l'energia che trasportano può essere portata più lontano rispetto ad onde compressive.
...
\begin{multicols}{2}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{immagini/riferimento.jpeg}
		\caption{cambio di sistema di riferimento $(\hat x, \hat y, \hat z) \ \ \rightarrow \ \ (\hat k, \hat y, \hat{\bar z})$}
		\label{fig:sist di rif}
	\end{figure}
	
	Per le trasformazioni fra vettori si usino le trasformazioni:
	\[\begin{pmatrix}
		k \\ y \\ \bar z
	\end{pmatrix}\ =\ 
	\begin{pmatrix}
	...
	Ad esempio si vede come nel sistema (x,y,z) il vettore d'onda sia $(k, 0, 0 )_{k,y,\bar z}\, =\, (k\sin\theta, 0, k\cos\theta)_{x,y,z} \equiv (k_x
	, 0 , k_z)_{x,y,z}$
\end{multicols}

\[  B_0\hat z\times (\vec k\times \vec b) \ =\ \begin{vmatrix} 
	\hat x & \hat y & \hat z \\ 
	0 & 0 & B_0\\
	-k_{z} b_y & - k b_{\bar z} & k_{x} b_y
\end{vmatrix} \ =\ \hat x\,(k\,B_0\, b_{\bar z})\, -\, \hat y (k_{z}\, b_y\, B_0) + \hat z(0)\]

Ne deriva che la seconda equazione del sitema sia:  

\[\omega \rho_0\vec u\ =\ c^2_S \vec k \rho_1\, +\, \frac{1}{4\pi}(kB_0 \vec b_{\bar z} \hat x - k_\parallel \vec b_y B_0 \hat y)\ \ .\] 

Si faccia attenzione al fatto che $\vec b_{\bar z} = -\cos\theta \vec b_x + \sin\theta \vec b_z$.
...
\[ \vec k\times (\vec u\times \vec B_0) \ =\ \begin{vmatrix} 
	\hat k & \hat y & \hat{\bar z} \\ 
	k & 0 & 0\\
	sin\theta B_0 u_y & B_0 u_x & -cos\theta B_0 u_y
\end{vmatrix} \ =\ -k_z B_0 u_y \hat y + k B_0 u_x \hat z\]

Il sistema di equazioni MHD, linearizzate ed in trasformata di Fourier, si riduce così a:
...
\begin{equation}
	...
	\begin{cases}
		...
		\frac{B_0}{4\pi}
		\begin{pmatrix}
			k (-\cos\theta b_x + \sin\theta b_z)\\ -k_z b_y\\ 0
		\end{pmatrix} 
		\\
		-\,\omega\,
		\begin{pmatrix}
		...
		\ =\ B_0\,
		\begin{pmatrix}
			k_z u_x\\ k_z u_y\\ -k_x u_x
		\end{pmatrix}  
	\end{cases}
\end{equation} 

Se ci limitiamo ad osservare la \underline{proiezione lungo $\hat y$} riduciamo il sistema a:
\[\begin{cases}
	\omega \rho_0 u_y\ =\ \displaystyle\frac{B_0}{4\pi}\, k_z b_y\\
	\omega b_y\ =\ -k_z B_0 u_y
\end{cases}\]
...
Otteniamo $\omega u_y = \omega^2 b_y / (k_\parallel B_0)$ e $\rho_0 \omega^2 b_y/(k_\parallel B_0) = B_0 k_\parallel b_y / (4\pi)$ che ci permettono di scrivere una \underline{relazione di dispersione per le onde di Alfvén $\forall\ \, \theta$}:

\begin{equation}
	\boxed{\ \omega^2\ =\ k^2_\parallel c^2_a\ =\ k^2\, cos^2\theta\, c^2_a\ }
\end{equation}

Notiamo che per $\theta = \pi/2$ non vi sono onde. Ricordiamo anche che si è usata la definizione di velocità di Alfvén $\boxed{c_a^2 \equiv B^2_0 / 4\pi \rho_0}$ (notare la differenza dalla già definita velocità di Alfvén $v_A \equiv B_0 / \sqrt{4\pi \rho}$).
...
Riprendiamo il sistema precedente, questa volta consideriamo la direzione perpendicolare a quella precedente, ignoriamo la direzione y e vediamo cosa succede nel piano $x-z$, stavolta troveremo le \textbf{onde magnetosoniche} che sono \underline{onde comprimibili}.

\[ \begin{cases}
	\omega \rho_1\ =\ \rho_0\ \vec k\cdot \vec u\ =\ \rho_0\ (k_\perp u_x + k_\parallel u_z)\\
	P_1\ =\ c^2_s\, \rho_1\ =\ \frac{\rho_0}{\omega}\ (k_\perp u_x + k_\parallel u_z)\\
	\omega\, \rho_0\, \vec u\ =\  c_s^2\, \vec k \rho_1 + \frac{\bar B_0}{4\pi}\hat z\times (\vec k\times \vec b)\\
	\omega\,\vec b\ =\ -\vec k\times (\vec u \times \vec B_0)
\end{cases} \]

Da questo sistema siamo partiti per arrivare al sitema eq.\ref{eq:onde MHD}, se si va a guardare le proiezioni lungo x-z questa volta troveremmo un sistema di 4 equazioni (due dal moto e due dall'equazione di Faraday). La differenza principale con il caso precedente è che il termine di pressione non si annulla quindi dobbiamo usare la politropica ed insieme l'equazione di continuità da cui ricavare $\rho_1$.

\[\begin{cases}
	\rho_0\omega u_x\ =\ c^2_s \rho_1 k_x\, +\, \frac{B_0}{4\pi} (k_x b_z - k_z b_x)\ \ ,\\
	\rho_0\omega u_x\ =\ c^2_s \rho_1 k_z\ \ ,\\
	\omega b_x\ =\ -\, B_0 k_z u_x\ \ ,\\
	\omega b_z\ =\ B_0 k_z u_x\ \ ,\\
	\omega \rho_1\ =\ \rho_0\, (k_x u_x + k_z u_z)\ \ .
\end{cases}\]

Se moltiplicassimo le prime due equazioni di questo sistema, provenienti dall'equazione del moto, per $\omega$ e al contempo dividessimo per $\rho_0$, vi riconosceremmo la velocità di Alfvén $c_a^2=B^2_0 /4\pi \rho_0$ e l'identità $k_x^2 + k_z^2 = k^2$:

\[\begin{cases}
	\omega^2 u_x\ =\ c^2_s k^2_x u_x\, +\, c^2_s k_x k_z u_z \, +\, c_a^2 k^2 u_x\ \ ,\\
	\omega^2 u_z\, -\, c^2_s k^2_z u_z\ =\ c^2_s k_x k_z u_x
\end{cases}\]

A questo punto è necessario ricavare una forma di $u_z$ dall'equazione della componente $\hat z$ per inserirla nell'equazione della componente $\hat x$, così da avere un'equazione del quarto grado da risolvere:

\[\omega^2 \cancel{u_x}\ =\ c^2_s k^2_x \cancel{u_x} \, +\, \frac{c^4_s k^2_x k^2_z }{\omega^2 - c^2_s k^2_z}\, \cancel{u_x}\, +\, c_a^2 k^2 \cancel{u_x}\]


\[ (\omega^2\, -\, k^2_x\, c^2_s\ -\ k^2\, c^2_a)\,(\omega^2\, -\, k^2_z\, c^2_s)\ +\ k^2_z\, k^2_x\, c^4_s\ =\ 0 \]

\[ \omega^4 \,-\, \omega^2\, k^2\, (c^2_a\, +\, c^2_s)\, +\, k^4\, \cos^2\theta\, c^2_a\, c^2_s\ =\ 0 \]
...
\[\boxed{\ \epsilon(k,\omega)\ =\ 1\,-\,\frac{\omega_e^2}{k^2}\int\frac{\partial_ug(u)}{(u-\dfrac{\omega}{k})}du\ =\ 0\ }\ \ \ .\] 
...
\[\epsilon(k,\omega_r+i\omega_i)\ \ \simeq\ \ \epsilon_r(k,\omega_r)\, +\, i\epsilon_i(k,\omega_r)\, +\, i\omega_i\frac{\partial \epsilon_r}{\partial\omega}\bigg|_{\omega=\omega_r}\, -\, \cancelto{\text{ordine successivo}}{\omega_i\frac{\partial \epsilon_i}{\partial\omega}\bigg|_{\omega=\omega_r}}\ \overset{!}{=}\ 0\ \ .\] 
