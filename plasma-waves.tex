...
\paragraph{LD emerge dal framework cinetico: riprendiamo l'eq. di Vlasov.}
...
Per quanto riguarda il termine $\vec E\cdot \vec \nabla_{\vec v}f_a$, che linearizziamo in $\frac{q_a}{m_a} (\vec E_0 + \vec E_1)\vec \nabla_{\vec v} (f_{s,0}+f_{s,1})$, le cose sono più delicate; è vero che all'istante iniziale $\displaystyle\frac{\partial f_{a0}}{\partial v}\gg\frac{\partial f_{a1}}{\partial v}$ ma i gradienti $\partial_v$ evolvono nel tempo e può arrivare un momento in cui la nostra linearizzazione non vale più e in gradienti evolvono in maniera da avere 
...
\[\int\, \frac{\partial_u\, g(u)}{(\frac{\omega}{k}-u)}\, \partial_u\]  
...
Questo mi permette di andare avanti nel calcolo come nel caso precedente assumendo che $v_f^2\gg v_{th}^2$ (ovvero $v_f>v_{th}$) e trovare un risultato completamente analogo 

\[\epsilon_r=1-\frac{\omega_e^2}{\omega^2}-\frac{3k^2v_{th}^2\omega_e^2}{\omega^4}\]

Questo è un caso un pochino borderline (\textit{"fa venire mal di pancia"}, concordo) perché non siamo nelle condizioni più favorevoli per avere il damping e la risonanza avviene con "pochi" elettroni. 
