...
\fboxrule=1pt
In un \underline{equilibrio "\emph{force-free}"}, dove non vi sono forze, si annulla il gradiente di pressione e deve valere $\boxed{\left(\vec \nabla \times \vec B \right)\times \vec B =0}$ (  $\boxed{\vec j\times \vec B =0}$ ).

Se $\left(\vec \nabla \times \vec B \right)\times \vec B =0$ allora i vettori dovrebbero essere paralleli e questo comporta che per un certo $\alpha$ siano proporzionali $\left(\vec \nabla \times \vec B \right)=\alpha \vec B$. 
